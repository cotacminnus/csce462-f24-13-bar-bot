...
class FacialRecognition:
    ...
    def capture_image(save_path="captured_image.jpg"):
        """
        Captures an image using `ffmpeg` and saves it to the specified path.
        Returns the path to the captured image.
        """
        try:
            print("Capturing image using ffmpeg...")
            save_dir = os.path.dirname(save_path)
            if save_dir and not os.path.exists(save_dir):
                os.makedirs(save_dir)

            # Use ffmpeg to capture the image
            subprocess.run(
                ["ffmpeg", "-f", "video4linux2", "-i", "/dev/video0", "-frames", "1", save_path],
