...
class FacialRecognition:
    ...
    def capture_image(save_path="captured_image.jpg"):
        ...
        """
        try:
            print("Capturing image with C270 webcam...")
            save_dir = os.path.dirname(save_path)
            if save_dir and not os.path.exists(save_dir):
                os.makedirs(save_dir)

            # Use fswebcam to capture the image with specific resolution and palette
            subprocess.run(
                ["fswebcam", "-d", "/dev/video0", "-r", "640x480", "--no-banner", "--palette", "YUYV", save_path],
                check=True
            )
    ...
    def detect_face(image_path):
        ...
        try:
            ...
            if edge_intensity > 200:  # Adjust threshold based on testing
...
class FacialRecognition:
    ...
    def detect_face(image_path):
        ...
        try:
            ...
            # Thresholds for detection
            if edge_intensity > 350:  # Adjust threshold based on testing
                print("Face-like structure detected!")
                return True
            else:
                print("No face detected.")
